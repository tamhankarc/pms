...
export async function getHRReportData(
  ...
  if (type === "attendance") {
    ...
    const years = Array.from(
      new Set(
        rangeDateKeys.map((dateKey) => Number(dateKey.slice(0, 4))),
      ),
    );
    ...
    for (const item of rowItems) {
      const existing: AttendanceExportEmployee = employeeMap.get(item.employeeId) ?? {
...
async function buildAttendanceWorkbook(data: HRReportData) {
  ...
  sheet.getCell(5, 2).value = "Default";
  ...
  for (const employee of matrix.employees) {
    if (employee.department !== currentDepartment) {
      ...
      sheet.addRow([]);
      const deptRow = sheet.addRow(["Department:", currentDepartment]);
    ...
    const totalRow = sheet.addRow([
      "Total",
      ...matrix.dateKeys.map((dateKey) => dayByKey.get(dateKey)?.total ?? "00:00"),
    ]);
  ...
  sheet.addRow([]);
  const footerRow = sheet.addRow(["Generated By: PMS EMS"]);
  footerRow.getCell(1).font = { bold: true };
  sheet.getColumn(1).width = 12;
  for (let index = 2; index <= totalColumns; index += 1) sheet.getColumn(index).width = 10;
  sheet.pageSetup = {
...
export async function buildHRReportWorkbook(data: HRReportData) {
  if (data.type === "attendance") return buildAttendanceWorkbook(data);
...
function pdfText(x: number, y: number, text: string | number, size = 7, bold = false) {
...
function pdfRect(x: number, y: number, width: number, height: number, lineWidth = 0.5) {
...
function pdfAttendanceEmployeeBlock(
  ...
  commands.push(pdfRect(x, tableTop - rowHeight * 4, totalWidth, rowHeight * 4));
  for (let row = 1; row < 4; row += 1) {
    const yLine = tableTop - row * rowHeight;
    commands.push(pdfLine(x, yLine, x + totalWidth, yLine, 0.35));
  }
  commands.push(pdfLine(x + labelWidth, tableTop, x + labelWidth, tableTop - rowHeight * 4, 0.35));
  for (let index = 1; index < dateKeys.length; index += 1) {
    const xLine = x + labelWidth + index * dayWidth;
    commands.push(pdfLine(xLine, tableTop, xLine, tableTop - rowHeight * 4, 0.25));
  }
  for (let row = 0; row < labels.length; row += 1) {
    ...
    for (let dayIndex = 0; dayIndex < dateKeys.length; dayIndex += 1) {
      commands.push(pdfText(x + labelWidth + dayIndex * dayWidth + 2, textY, values[row][dayIndex], 5.7));
    }
  }
  return commands;
}

function buildAttendancePdf(data: HRReportData) {
  const matrix = data.attendanceMatrix;
  if (!matrix) throw new Error("Attendance report data is not available.");
  const pageWidth = 842;
  ...
  const margin = 22;
  const labelWidth = 44;
  const dayWidth = Math.min(28, Math.max(18, (pageWidth - margin * 2 - labelWidth) / Math.max(matrix.dateKeys.length, 1)));
  ...
  const addHeader = (pageNumber: number) => {
    commands.push(pdfText(300, pageHeight - 22, "Monthly Status Report (Basic Work Duration)", 11, true));
    commands.push(pdfText(355, pageHeight - 48, data.periodLabel, 8));
    commands.push(pdfText(margin, pageHeight - 84, "Company:", 7, true));
    commands.push(pdfText(margin + 55, pageHeight - 84, "Default", 7));
    commands.push(pdfText(pageWidth - 170, pageHeight - 84, "Printed On:", 7, true));
    commands.push(pdfText(pageWidth - 100, pageHeight - 84, printedOn, 7));
    commands.push(pdfLine(margin, pageHeight - 92, pageWidth - margin, pageHeight - 92));
    ...
    commands.push(pdfLine(margin + labelWidth, headerTop, margin + labelWidth, headerTop - 16, 0.35));
    matrix.dateKeys.forEach((dateKey, index) => {
      const cellX = margin + labelWidth + index * dayWidth;
      if (index > 0) commands.push(pdfLine(cellX, headerTop, cellX, headerTop - 16, 0.25));
      commands.push(pdfText(cellX + 2, headerTop - 10, dayLabel(dateKey), 5.7));
    });
    commands.push(pdfLine(margin, 38, pageWidth - margin, 38));
    commands.push(pdfText(margin, 22, "Generated By: PMS EMS", 7));
  ...
  let currentDepartment = "";
  for (const employee of matrix.employees) {
    const needsDepartment = employee.department !== currentDepartment;
    const blockHeight = (needsDepartment ? 18 : 0) + 84;
    if (y - blockHeight < 52) {
      pages.push(commands);
      commands = [];
      pageNumber += 1;
    ...
    if (employee.department !== currentDepartment) {
      currentDepartment = employee.department;
      commands.push(pdfText(margin, y, "Department:", 7, true));
      commands.push(pdfText(margin + 68, y, currentDepartment, 7));
      y -= 22;
    }
    commands.push(...pdfAttendanceEmployeeBlock(employee, matrix.dateKeys, margin, y, labelWidth, dayWidth));
    y -= 82;
  }
  pages.push(commands);
...
export function buildHRReportPdf(data: HRReportData) {
  if (data.type === "attendance") return buildAttendancePdf(data);
