...
export async function createAssetTypeAction(_prevState: AssetTypeFormState, formData: FormData): Promise<AssetTypeFormState> {
  try {
    ...
    const parsed = assetTypeSchema.safeParse({ clientId: formData.get("clientId"), name: formData.get("name"), description: formData.get("description") || "", isActive: formData.get("isActive") ?? "on" });
    ...
    await db.assetType.create({ data: { clientId: parsed.data.clientId, name: parsed.data.name.trim(), code: generatedCode, description: parsed.data.description?.trim() || null, isActive: Boolean(parsed.data.isActive) } });
...
export async function updateAssetTypeAction(_prevState: AssetTypeFormState, formData: FormData): Promise<AssetTypeFormState> {
  try {
    await requireUserTypesForAction(["ADMIN", "MANAGER", "TEAM_LEAD"]);
    const parsed = assetTypeSchema.safeParse({ id: formData.get("id"), clientId: formData.get("clientId"), name: formData.get("name"), description: formData.get("description") || "", isActive: formData.get("isActive") ?? undefined });
    ...
    await db.assetType.update({ where: { id: parsed.data.id }, data: { clientId: parsed.data.clientId, name: parsed.data.name.trim(), code, description: parsed.data.description?.trim() || null, isActive: Boolean(parsed.data.isActive) } });
