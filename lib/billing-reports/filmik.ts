...
import { getExportTimestamp, sanitizeFileSegment } from "@/lib/billing-reports/amazon";
...
export function buildFilmikBillingReportFilters(searchParams: URLSearchParams | Record<string, string | string[] | undefined>) {
  const value = searchParams instanceof URLSearchParams ? searchParams.get("month") : Array.isArray(searchParams.month) ? searchParams.month[0] : searchParams.month;
...
function formatMonthLabel(month: string) {
  ...
  return from.toLocaleString("en-US", { month: "long", year: "numeric", timeZone: "UTC" });
}
...
function latestCountsForMonth<TRow extends { projectId: string; resourceTypeId: string; effectiveMonth: Date; count: number }>(rows: TRow[], monthStart: Date) {
  ...
  for (const row of rows) {
    ...
    if (!existing || row.effectiveMonth > existing.effectiveMonth || (row.effectiveMonth.getTime() === existing.effectiveMonth.getTime())) latest.set(key, row);
  }
  return latest;
}

export async function getFilmikBillingReportData(filters: FilmikBillingReportFilters): Promise<FilmikBillingReportData | null> {
  ...
  const [resourceTypes, countHistory, projectTimeGroups] = await Promise.all([
    ...
    db.projectFilmikResourceCount.findMany({
      where: { project: { clientId: FILMIK_CLIENT_ID }, effectiveMonth: { lte: from } },
      select: { projectId: true, resourceTypeId: true, effectiveMonth: true, count: true },
      orderBy: [{ resourceTypeId: "asc" }, { projectId: "asc" }, { effectiveMonth: "asc" }],
    }),
    db.timeEntry.groupBy({
      by: ["projectId"],
      where: { project: { clientId: FILMIK_CLIENT_ID }, workDate: { gte: from, lte: to } },
  ...
  for (const row of latest.values()) {
    ...
    countByResource.set(row.resourceTypeId, (countByResource.get(row.resourceTypeId) ?? 0) + row.count);
  }
  ...
  const resourceTotalCount = resourceRows.reduce((sum, row) => sum + row.count, 0);
  const resourceTotalCost = resourceRows.reduce((sum, row) => sum + row.cost, 0);
  const minutesByProject = new Map(projectTimeGroups.map((group) => [group.projectId, group._sum.minutesSpent ?? 0]));
  ...
  const combinedRows: FilmikCombinedReportRow[] = [
    { key: "resource-cost", name: "Resource Cost", quantity: resourceTotalCount, cost: resourceTotalCost, contactPerson: "-" },
    ...projectRows,
  ];
  const combinedTotalCost = combinedRows.reduce((sum, row) => sum + row.cost, 0);
...
export function getFilmikBillingReportFileName(data: FilmikBillingReportData, extension: "xls" | "pdf") {
...
export function getFilmikBillingReportMonthLabel(data: FilmikBillingReportData) {
