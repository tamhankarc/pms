...
export function ContactPersonForm({ clients, action, initialValues, submitLabel, title }: {
  clients: Client[];
  action: (state: ContactPersonFormState, formData: FormData) => Promise<ContactPersonFormState>;
  initialValues?: { id?: string; clientId: string; name: string; email: string; contactNumber: string | null; };
  submitLabel: string;
  title: string;
}) {
  ...
  return (
    <form action={formAction} className="card p-6">
      ...
      <div className="mt-5 space-y-4">
        <div><FormLabel htmlFor="clientId" required>Client</FormLabel><SearchableCombobox id="clientId" options={clientOptions} value={selectedClientId} onValueChange={setSelectedClientId} placeholder="Select client" searchPlaceholder="Search clients..." emptyLabel="No client found." /></div>
