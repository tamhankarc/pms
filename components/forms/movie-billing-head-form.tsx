...
export function MovieBillingHeadForm({ clients, action, initialValues, submitLabel, title }: {
  ...
  initialValues?: { id?: string; clientId: string; name: string; compulsionType: CompulsionType; costType: CostType; domesticCost: string | number; intlCost: string | number; isActive: boolean };
  submitLabel: string;
  title: string;
}) {
  const [state, formAction, pending] = useActionState(action, initialState);
  const [clientId, setClientId] = useState(initialValues?.clientId ?? "");
  const [compulsionType, setCompulsionType] = useState<CompulsionType>(initialValues?.compulsionType ?? "FIXED_COMPULSORY");
  ...
  return (
    <form action={formAction} className="card p-6">
      ...
      <input type="hidden" name="compulsionType" value={compulsionType} />
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <div><FormLabel htmlFor="compulsionType" required>Head type</FormLabel><SearchableCombobox id="compulsionType" value={compulsionType} onValueChange={(v) => setCompulsionType(v as CompulsionType)} options={[{ value: "FIXED_COMPULSORY", label: "Fixed - Compulsory" }, { value: "FIXED_OPTIONAL", label: "Fixed - Optional" }]} placeholder="Select type" searchPlaceholder="Search types..." emptyLabel="No type found." required /></div>
        <div><FormLabel htmlFor="costType" required>Cost type</FormLabel><SearchableCombobox id="costType" value={costType} onValueChange={(v) => setCostType(v as CostType)} options={[{ value: "WHOLE_COST", label: "Whole cost" }, { value: "PER_UNIT_COST", label: "Per-unit cost" }]} placeholder="Select cost type" searchPlaceholder="Search cost types..." emptyLabel="No cost type found." required /></div>
        <div><FormLabel htmlFor="domesticCost" required>Domestic cost (USD)</FormLabel><div className="relative"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium text-slate-500">$</span><input id="domesticCost" name="domesticCost" type="number" min="0" step="0.01" className="input pl-7" defaultValue={initialValues?.domesticCost ?? "0.00"} required /></div></div>
        <div><FormLabel htmlFor="intlCost" required>INTL cost (USD)</FormLabel><div className="relative"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium text-slate-500">$</span><input id="intlCost" name="intlCost" type="number" min="0" step="0.01" className="input pl-7" defaultValue={initialValues?.intlCost ?? "0.00"} required /></div></div>
