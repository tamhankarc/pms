  ...
  return (
    <form action={formAction} className="card p-6">
      ...
      <h2 className="section-title">
        {mode === "create" ? "Create employee group" : "Edit employee group"}
      </h2>
      ...
      <div className="mt-5 space-y-4">
        ...
        <div>
          ...
          <div className="mt-3 max-h-72 overflow-y-auto rounded-xl border border-slate-200 bg-white p-3">
            <div className="grid gap-2 md:grid-cols-2">
              {filteredUsers.map((user) => {
                const checked = (initialValues?.userIds ?? []).includes(
                  user.id,
                );
                return (
                  <label
          ...
          <p className="mt-1 text-xs text-slate-500">
            Only Employees and Team Leads are available here. Use the checkboxes
            to pick members quickly.
          </p>
        </div>
