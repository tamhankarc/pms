...
export default async function BillingContactsPage({
  ...
  const [clients, contactPersons, projects, assignments] = await Promise.all([
    ...
    db.billingContactAssignment.findMany({
      include: {
        client: true,
        contactPerson: true,
        project: true,
  ...
  const catalogReports = clients.flatMap((client: { id: string; name: string }) => {
    ...
    return Object.entries(catalog).map(([value, item]) => ({
      value,
      label: item.title,
    }));
  });
  ...
  const reportMap = new Map(
    [...fallbackReports, ...catalogReports].map((item) => [item.value, item]),
  );
  const reports = Array.from(reportMap.values()).sort((a, b) =>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Assign Bill To contacts for Client, Client + Project, or Client + Billing Report."
        actions={
      ...
      <BillingContactForm
          ...
          projects={projects}
          reports={reports}
          action={saveBillingContactAction}
        />
      </div>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr>
              ...
              <th className="table-head">Project / Report</th>
              <th className="table-head">Bill To</th>
              <th className="table-head">Actions</th>
            </tr>
          </thead>
          <tbody>
            {assignments.map((assignment: BillingContactAssignmentRow) => (
              <tr key={assignment.id}>
                <td className="table-cell font-medium text-slate-900">
                  {assignment.client.name}
                </td>
                <td className="table-cell">{formatLevel(assignment.assignmentLevel)}</td>
                <td className="table-cell">
                  {assignment.project?.name ??
                    (assignment.billingReportType
                      ? reportMap.get(assignment.billingReportType)?.label ??
                        assignment.billingReportType
                      : "All reports")}
                </td>
                <td className="table-cell">
                  Bill To: {assignment.contactPerson.name} ({assignment.contactPerson.email})
                </td>
                <td className="table-cell">
                  <form action={deleteBillingContactAction}>
                    <input type="hidden" name="id" value={assignment.id} />
                    <DeleteButton confirmMessage="Delete this billing contact assignment?" />
                  </form>
                </td>
              </tr>
            ))}
