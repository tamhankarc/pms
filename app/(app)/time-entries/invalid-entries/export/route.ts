...
export async function GET(request: Request) {
  ...
  const invalidEntries = filterInvalidTimeEntries(entries).map((row) => row.entry);
  const buffer = await buildTimeEntriesWorkbook({ entries: invalidEntries, invalidOnly: true });

  return new Response(buffer, {
    headers: {
      "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "Content-Disposition": `attachment; filename="invalid_time_entries.xlsx"`,
