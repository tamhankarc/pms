...
export default async function ProjectDetailPage({
  ...
  const [projectSpecialTypes] = await db.$queryRaw<
    Array<{
      warnerProjectType?: "OTHER" | "PORTAL" | "DVD" | "TICKETING" | "SOCIAL" | null;
  ...
  const filmikResourceTypes = filmikResourceTypesRaw.map((resource) => ({
    ...
    latestMonth: resource.projectCounts[0]?.effectiveMonth
      ? resource.projectCounts[0].effectiveMonth.toISOString().slice(0, 7)
      : "",
  }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Update project profile, billing model, entry dropdown behavior, and billing report cost fields."
        actions={
        ...
        initialValues={{
          ...
          fixedContractHours:
            project.fixedContractHours == null
              ? null
              : Number(project.fixedContractHours),
          fixedMonthlyHours:
            project.fixedMonthlyHours == null
              ? null
              : Number(project.fixedMonthlyHours),
