...
function assignedPeopleForProject(
  project: Awaited<ReturnType<typeof getVisibleProjects>>[number],
) {
  return Array.from(
...
function filterProjects(
  ...
  return projects.filter((project) => {
    if (kind === "fixedFull" && project.billingModel !== "FIXED_FULL")
      return false;
    if (kind === "standard" && project.billingModel === "FIXED_FULL")
      return false;
    ...
    const matchesBilling =
      filters.billingModel === "all"
        ? true
        : project.billingModel === filters.billingModel;
    const matchesClient =
      filters.clientId === "all" ? true : project.clientId === filters.clientId;
...
function projectTableRows(
  ...
  estimateCounts: Map<string, number>,
  showEstimateCount: boolean,
  allowDelete: boolean,
) {
...
export default async function ProjectsPage({
  ...
  const allowDelete = canDeleteProjects(user);
  const standardFilters = {
    q: params.q ?? "",
    status: params.status ?? "all",
    billingModel: params.billingModel ?? "all",
    clientId: params.clientId ?? "all",
  };
  const fixedFilters = {
    q: params.ff_q ?? "",
    status: params.ff_status ?? "all",
    billingModel: "FIXED_FULL",
    clientId: params.ff_clientId ?? "all",
  };
  ...
  const estimateCounts = new Map(
    estimateCountRows.map((row) => [row.projectId, row.count]),
  );
  const standardProjects = filterProjects(
    allProjects,
    standardFilters,
    "standard",
  );
  const fixedFullProjects = filterProjects(
    allProjects,
    fixedFilters,
    "fixedFull",
  );
  const standardPaged = paginateItems(
    standardProjects,
    standardPage,
    DEFAULT_PAGE_SIZE,
  );
  const fixedPaged = paginateItems(
    fixedFullProjects,
    fixedPage,
    DEFAULT_PAGE_SIZE,
  );
      ...
      <div id="standard-projects" className="space-y-4 scroll-mt-6">
        ...
        <div className="card p-4">
          <AutoSubmitFilterForm
            className="grid gap-3 md:grid-cols-[1fr_180px_220px_220px_auto]"
            method="get"
            action="/projects#standard-projects"
          >
            <input type="hidden" name="ff_q" value={fixedFilters.q} />
            <input type="hidden" name="ff_status" value={fixedFilters.status} />
            <input
              type="hidden"
              name="ff_clientId"
              value={fixedFilters.clientId}
            />
            <input
              className="input"
              name="q"
              defaultValue={standardFilters.q}
              placeholder="Search by project, client, or assigned person"
            />
            <SearchableCombobox
              id="status"
              name="status"
              defaultValue={standardFilters.status}
            ...
            <SearchableCombobox
              id="billingModel"
              name="billingModel"
              defaultValue={standardFilters.billingModel}
              options={billingOptions}
              placeholder="All billing models"
              searchPlaceholder="Search billing models..."
              emptyLabel="No billing models found."
            />
            <SearchableCombobox
              id="clientId"
              name="clientId"
              defaultValue={standardFilters.clientId}
        ...
        <div className="table-wrap">
          ...
          <PaginationControls
            basePath="/projects"
            anchor="#standard-projects"
            currentPage={standardPaged.currentPage}
            totalPages={standardPaged.totalPages}
            totalItems={standardPaged.totalItems}
            pageSize={standardPaged.pageSize}
            searchParams={{
              ...standardFilters,
              ff_q: fixedFilters.q,
              ff_status: fixedFilters.status,
              ff_clientId: fixedFilters.clientId,
            }}
          />
        </div>
      </div>
      <div id="fixed-full-projects" className="mt-8 space-y-4 scroll-mt-6">
        ...
        <div className="card p-4">
          <AutoSubmitFilterForm
            ...
            method="get"
            action="/projects#fixed-full-projects"
          >
            <input type="hidden" name="q" value={standardFilters.q} />
            <input type="hidden" name="status" value={standardFilters.status} />
            <input
              type="hidden"
              name="billingModel"
              value={standardFilters.billingModel}
            />
            <input
              type="hidden"
              name="clientId"
              value={standardFilters.clientId}
            />
            <input
              className="input"
              name="ff_q"
              defaultValue={fixedFilters.q}
              placeholder="Search by project, client, or assigned person"
            />
            <SearchableCombobox
              id="ff_status"
              name="ff_status"
              defaultValue={fixedFilters.status}
              options={statusOptions}
              placeholder="All statuses"
              searchPlaceholder="Search statuses..."
              emptyLabel="No statuses found."
            ...
            <SearchableCombobox
              id="ff_clientId"
              name="ff_clientId"
              defaultValue={fixedFilters.clientId}
              options={clientComboOptions}
              placeholder="All clients"
              searchPlaceholder="Search clients..."
              emptyLabel="No clients found."
        ...
        <div className="table-wrap">
          <table className="table-base">
            <thead className="table-head">
              <tr>
                ...
                <th className="table-cell">Assigned People</th>
                <th className="table-cell">Billing</th>
                <th className="table-cell">Estimates</th>
                <th className="table-cell">Status</th>
                <th className="table-cell">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              ...
              {fixedFullProjects.length === 0 ? (
                <tr>
                  <td
                    colSpan={7}
                    className="table-cell text-center text-sm text-slate-500"
                  >
                    No Fixed Full projects found.
                  </td>
                </tr>
              ) : null}
            </tbody>
          ...
          <PaginationControls
            basePath="/projects"
            pageParam="ff_page"
            anchor="#fixed-full-projects"
            currentPage={fixedPaged.currentPage}
            totalPages={fixedPaged.totalPages}
            totalItems={fixedPaged.totalItems}
            pageSize={fixedPaged.pageSize}
            searchParams={{
              q: standardFilters.q,
              status: standardFilters.status,
              billingModel: standardFilters.billingModel,
              clientId: standardFilters.clientId,
              ff_q: fixedFilters.q,
              ff_status: fixedFilters.status,
              ff_clientId: fixedFilters.clientId,
            }}
