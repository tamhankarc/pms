...
export default async function CopyDecksPage({ searchParams }: {
  searchParams?: Promise<{ clientId?: string; movieId?: string; projectId?: string; subProjectId?: string; countryId?: string; page?: string }>;
}) {
  ...
  const countryId = clientId ? (params.countryId || options.australiaId) : options.australiaId;
  const decks = await db.copyDeck.findMany({
    where: {
      ...(clientId ? { clientId, countryId } : {}),
    ...
    include: { client: true, movie: true, project: true, subProject: true, country: true, createdBy: true, _count: { select: { rows: true } } },
  ...
  return <div className="space-y-6">
    ...
    <CopyDeckFilterForm {...options} initial={{ clientId, movieId: params.movieId ?? "", projectId: params.projectId ?? "", subProjectId: params.subProjectId ?? "", countryId }} />
    <div className="table-wrap"><table className="table-base"><thead className="table-head"><tr>
      <th className="table-cell">Name</th><th className="table-cell">Client</th><th className="table-cell">Title / Project</th><th className="table-cell">Country</th><th className="table-cell">Rows</th><th className="table-cell">Created</th><th className="table-cell">Action</th>
      ...
      {page.items.map((deck) => <tr key={deck.id}>
        ...
        <td className="table-cell">{deck.country.name}</td><td className="table-cell">{deck._count.rows}</td>
