...
export default async function ContactPersonEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [clients, contactPerson] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={`Edit Contact Person · ${contactPerson.name}`} description="Update client contact person details." actions={<Link href="/contact-persons" className="btn-secondary">Back to Contact Persons</Link>} />
      <div className="max-w-3xl"><ContactPersonForm clients={clients} action={updateContactPersonAction} title={`Edit Contact Person: ${contactPerson.name}`} submitLabel="Save changes" initialValues={{ id: contactPerson.id, clientId: contactPerson.clientId, name: contactPerson.name, email: contactPerson.email, contactNumber: contactPerson.contactNumber }} /></div>
    </div>
  );
}
