...
import {
  deleteMovieBillingHeadAssignmentAction,
  toggleMovieBillingHeadAssignmentStatusAction,
} from "@/lib/actions/movie-billing-head-assignment-actions";
...
export default async function MovieBillingHeadsPage({
  ...
  const canDeleteRows = canDeleteMovieBillingHeads(currentUser);
        ...
        movieBillingHeads: {
          some: {
            ...
            OR: [
              ...
              { otherActive: true, otherCompulsionType: "FIXED_OPTIONAL" },
            ],
          },
        },
      },
  ...
  return (
    <div>
      <PageHeader
        title="Title Billing Heads"
        description="Assign Fixed - Optional billing heads to Working titles by client and country."
        actions={
          <Link href="/movie-billing-heads/new" className="btn-primary">
            Create Title Billing Head
          </Link>
        }
      />
      ...
      <div className="mb-6 card p-4">
        <MovieBillingHeadAssignmentListFilters
          q={q}
          clientId={clientId}
          movieId={movieId}
          status={status}
      ...
      <div className="table-wrap">
        <table className="table-base">
          <thead className="table-head">
            <tr>
              ...
              {canSeeCosts ? <th className="table-cell">Units</th> : null}
          ...
          <tbody className="divide-y divide-slate-100">
            {items.map((row) => (
              <tr key={row.id}>
                ...
                {canSeeCosts ? (
                  <td className="table-cell">
                    {row.billingHead.costType === "PER_UNIT_COST"
                      ? "Per-unit cost"
                      : "Whole cost"}
                  </td>
                ) : null}
        ...
        <PaginationControls
          basePath="/movie-billing-heads"
          currentPage={currentPage}
          totalPages={totalPages}
          totalItems={totalItems}
          pageSize={pageSize}
          searchParams={{ q, clientId, movieId, status }}
        />
      </div>
    </div>
  );
