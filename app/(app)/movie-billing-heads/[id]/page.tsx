...
export default async function EditMovieBillingHeadPage({
  ...
  const [clients, countries, movies, billingHeads, assignmentGroupRows] =
    await Promise.all([
      db.client.findMany({
        where: {
          isActive: true,
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <PageHeader
        title={`Edit Title Billing Head · ${row.billingHead.name}`}
        description="Update the Fixed - Optional billing head selected for this client movie."
        actions={
          <Link href="/movie-billing-heads" className="btn-secondary">
            Back to Title Billing Heads
          </Link>
        }
      />
      {row.movie.status === "COMPLETED_BILLED" ? (
        <div className="card p-6">
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            This title billing head belongs to the billed title{" "}
            <strong>{row.movie.title}</strong>. It is viewable but cannot be
            edited.
          </div>
          <dl className="mt-5 grid gap-4 md:grid-cols-2">
            <div>
              ...
              <dd className="mt-1 text-sm text-slate-900">{row.movie.title}</dd>
            </div>
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">
                Billing Head
            ...
            <div>
              ...
              <dd className="mt-1 text-sm text-slate-900">
                {row.units ? Number(row.units).toString() : "-"}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">
