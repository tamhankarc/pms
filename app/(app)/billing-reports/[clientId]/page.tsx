    ...
    <div className="table-wrap">
      <table className="table-base">
        ...
        <tbody className="divide-y divide-slate-100">
          ...
          {data.reportType === "other-deliverable" && data.selectedMovie && !data.selectedCountry ? <tr><td colSpan={2} className="table-cell text-center text-sm text-slate-500">Select a country with time entries for the selected movie to view deliverables.</td></tr> : null}
          {data.selectedMovie && (data.reportType !== "other-deliverable" || data.selectedCountry) && groups.map((group) => {
          ...
          {data.selectedMovie && (data.reportType !== "other-deliverable" || data.selectedCountry) ? <tr className="bg-slate-100"><td className="table-cell font-semibold text-slate-900">Total</td><td className="table-cell font-semibold text-slate-900">{formatUsd(data.totalCost)}</td></tr> : null}
        </tbody>
      </table>
    </div>
  );
...
function GenericBillingReportFilters({ clientId, reportType, data }: { clientId: string; reportType?: AmazonReportType; data: GenericBillingReportData }) {
  return (
    <form method="get" action={`/billing-reports/${clientId}`} className="card p-5">
      {reportType ? <input type="hidden" name="report" value={reportType} /> : null}
      <div className={data.movieSpecific ? "grid gap-4 md:grid-cols-[180px_180px_1fr_auto] md:items-end" : "grid gap-4 md:grid-cols-[180px_180px_auto_1fr] md:items-end"}>
        <div>
          <label className="label" htmlFor="fromDate">Date from</label>
          <input id="fromDate" name="fromDate" type="date" className="input" defaultValue={data.filters.fromDate} />
        </div>
        ...
        {data.movieSpecific ? (
          <div>
            ...
            <SearchableCombobox id="movieId" name="movieId" defaultValue={data.filters.movieId} options={data.movieOptions.map((movie) => ({ value: movie.id, label: movie.title }))} placeholder="Select movie" searchPlaceholder="Search movies..." emptyLabel="No movies found." />
          </div>
        ) : null}
        <button className="btn-primary" type="submit">Apply</button>
        {!data.movieSpecific ? <p className="text-sm text-slate-500 md:text-right">Date range is used for Hourly project costs.</p> : null}
      </div>
      {data.movieSpecific ? <p className="mt-3 text-sm text-slate-500">Date range is used for Hourly project costs. Report rows are limited to projects with time entries for the selected movie.</p> : null}
    </form>
  );
}

function GenericExportButtons({ clientId, reportType, data }: { clientId: string; reportType?: AmazonReportType; data: GenericBillingReportData }) {
  const query = buildQueryString({
    report: reportType ?? "",
...
function GenericBillingReportWorkspace({ clientId, reportType, data, clientName }: { clientId: string; reportType?: AmazonReportType; data: GenericBillingReportData; clientName?: string }) {
  return (
    <div className="space-y-6">
      {reportType && clientName ? <ReportTabs clientId={clientId} activeReport={reportType} clientName={clientName} /> : null}
      <GenericBillingReportFilters clientId={clientId} reportType={reportType} data={data} />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <GenericExportButtons clientId={clientId} reportType={reportType} data={data} />
      </div>
...
export default async function ClientBillingReportPage({ params, searchParams }: { params: Promise<{ clientId: string }>; searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const genericBillingOptions = activeReportDefinition?.kind === "generic-movie"
    ...
    : activeReportDefinition?.kind === "generic-filmik"
      ...
      : undefined;
  const genericBillingReportData = !reportCatalog || genericBillingOptions
    ? await getGenericBillingReportData({ clientId, filters: genericFilters, options: genericBillingOptions })
    : null;
